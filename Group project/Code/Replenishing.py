...
class Replenishing:
    ...
    def __init__(self, warehouse):
        ...
        self.simu_inventory = copy.deepcopy(warehouse.InventoryInfo) 
        self.warehouse = copy.deepcopy(warehouse.InventoryInfo)
        self.initialize()
    ...
    def initialize(self):
        ...
        for i in order.OrdersInfo:
            keys.append(i)
        order_1 = order.OrdersInfo[keys[0]]
        self.order_list = order_1
        order_2 = order.OrdersInfo[keys[1]]
        for i in order_1:
    ...
    def RushOrder(self, warehouse):
        ...
        while input_validation:   
            ...
            d = input('Enter the category number: ')
            c = int(d[4:6])
            if c not in range(100):  
                ...
                while input_validation:
                    ...
                    if num > int(self.simu_inventory[c]['CurrentInventory']):   
                        ...
                        self.order_list.append({'Contact': user, 'ContactNumber': contact, 'EstDeliverDate': delivery_dt, 'ID':str(len(self.order_list)+1), 'ItemName': name, 'ItemNumber': d, 'OrderAmount': str(num), 'OrderDate': delivery_dt.date(), 'OrderNumber': 'R{}'.format(self.rush_order_num), 'Specification': sp, 'SupermarketName': bn[:2] + bn[4],'SupermarketNumber': bn})
                        if if_more == 'Y':
                            break
